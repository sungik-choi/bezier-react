...
import useProgressiveImage, { CacheImage } from './useProgressiveImage'
...
describe('useProgressiveImage >', () => {
  ...
  const mockImageCache = new Map<string, CacheImage>()
  ...
  it('should update url when imageCache.get(url).isLoaded is "true"', () => {
    const { result, rerender } = renderHook(({ src, defaultSrc, imageCache }) => useProgressiveImage(src, defaultSrc, imageCache), {
      initialProps: {
